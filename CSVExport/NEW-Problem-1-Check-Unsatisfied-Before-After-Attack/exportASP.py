...
class ASPExporter:
    ...
    #takes a filepath as _, does all operations up to actual exportation
    def __init__(self,filename,version):
        
        
        if(version == "NEWSR01"):
            
            self.aspectPattern = "aspect(\"cpsf:{}\")"
            self.concernPattern = " concern(\"cpsf:{}\")"
            self.subconcernPattern = " subconcern(\"cpsf:{}\",\"cpsf:{}\""
            self.satisfiedPattern = " h(sat(\"cpsf:{}\"),0)"
            self.unsatisfiedPattern = " -h(sat(\"cpsf:{}\"),0)"
        
        elif(version == "NEWSR02"):
            
            self.aspectPattern = "aspect({})"
            self.concernPattern = "concern({})"
            self.subconcernPattern = " subconcern({},{})"
            self.satisfiedPattern = " h(sat({}),0)"
            self.unsatisfiedPattern = " -h(sat({}),0)"
        
        else:
            
            print("Passed version not valid, try NEWSR01,NEWSR02,etc")
    ...
    def addAllAspects(self):
        ...
        parsed_aspects = ' '.join(r[0] for r in findall(self.aspectPattern, self.encodingLine))
    ...
    def addAllConcerns(self):
        ...
        parsed_concerns = ' '.join(r[0] for r in findall(self.concernPattern, self.encodingLine))
        ...
        for concern in allConcerns_strList:
            ...
            if((self.getCSVNode(concern) != 0) or concern == "all"):
                continue
    ...
    def addAllSubconcernRelations(self):
        ...
        subconcerns_parse = ' '.join(r[0] for r in findall(self.subconcernPattern, self.encodingLine))
        subconcerns_parse2 = ' '.join(r[1] for r in findall(self.subconcernPattern, self.encodingLine))
    ...
    def setSatisfaction(self):
        ...
        parsed_satisfied =  ' '.join(r[0] for r in findall(self.satisfiedPattern, self.encodingLine))
        parsed_unsatisfied =  ' '.join(r[0] for r in findall(self.unsatisfiedPattern, self.encodingLine))
...
def exportCSV(input_file_path,output_file_path,version):
    
    print("loading from ", input_file_path)
    exporter = ASPExporter(input_file_path,version)
...
exportCSV("./SR-02/SR_02_use_case_2_LKAS_Case_1.txt","./SR-02/usecase2_LKAS_case1.csv","NEWSR02")
