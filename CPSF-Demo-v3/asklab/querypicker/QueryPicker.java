...
//	private int[] solverConst = { CPSReasoner.SLVR_DLV, CPSReasoner.SLVR_DLV_ALL, CPSReasoner.SLVR_CLINGO, CPSReasoner.SLVR_CLINGO_ALL };
	...
	public QExecActionListener(String sparqlFile,String baseDir,String exDir,String aspFile,JProgressBar progr,int solver,JTextArea taRes)
...
public class QueryPicker
{
	...
	public JButton createQueryButton(String l,String sparqlFile,String baseDir,String exDir,String aspFile,JProgressBar progressBar,int solver,JTextArea taRes)
	...
	public void loadContent(JPanel taQs,JProgressBar progressBar,JTextArea taRes) throws FileNotFoundException, IOException
	{	String[] content=Utils.readFile(contentFile).split("\\r?\\n");
		for (int l=0;l<content.length;l++)
 		{	String line=content[l];
			...
			if (line.toUpperCase().equals("*QUERY"))
			{	if (content.length-l<=3)
				...
				if (solverIndex==-1)
				{	solverIndex=0;
					System.out.println("***WARNING: invalid solver name "+solverName+" in "+contentFile+". Defaulting to "+solverStrings[solverIndex]);
				}	
...
//		rightpan.add(GUIHelper.makeBorder(spRes,"Result"));
		...
		JSplitPane splitPane = 
			new JSplitPane(JSplitPane.VERTICAL_SPLIT,
				rightpan, GUIHelper.makeBorder(spRes,"Result"));
